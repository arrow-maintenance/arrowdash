...
# Identify when each contributor first got a PR merged.
first_merged_prs <- prs |>
  filter(!grepl("\\[bot\\]", user_login, ignore.case = TRUE)) |>
  filter(!is.na(merged_at)) |>
  group_by(user_login) |>
  slice_min(merged_at, n = 1, with_ties = FALSE) |>
  ungroup() |>
  transmute(user_login, first_merged_date = as.Date(merged_at))
...
pr_contributors <- prs |>
  ...
  mutate(
    contributor_type = if_else(
      is.na(first_merged_date) | created_date < first_merged_date,
      "New contributors",
...
new_contributor_prs <- prs %>%
  left_join(first_merged_prs, by = "user_login") %>%
  mutate(created_date = as.Date(created_at)) %>%
  filter(is.na(first_merged_date) | created_date < first_merged_date) %>%
